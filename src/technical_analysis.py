...
class TechnicalAnalyzer:
    ...
    def __init__(self):
        ...
        if not pd or not np:
            raise ImportError("Установите pandas и numpy: pip install pandas numpy")
    ...
    def interpret_rsi(self, rsi_value: float) -> Dict[str, str]:
        ...
        if rsi_value >= 70:
            return {"signal": "SELL", "description": "Перекупленность (RSI ≥ 70)", "emoji": "🔴"}
        elif rsi_value <= 30:
            return {"signal": "BUY", "description": "Перепроданность (RSI ≤ 30)", "emoji": "🟢"}
        elif rsi_value >= 50:
            return {
    ...
    def interpret_macd(self, macd_data: Dict[str, pd.Series]) -> Dict[str, str]:
        ...
        try:
            ...
            return {"signal": "UNKNOWN", "description": "Ошибка анализа MACD", "emoji": "⚪"}
    ...
    async def get_technical_analysis(self, ticker: str) -> Dict:
        ...
        try:
            ...
            result = {
                "ticker": ticker,
                "company_name": {"name": f"Акция {ticker}", "sector": "Неизвестно"}.get(
                    "name", f"Акция {ticker}"
                ),
                ...
                "indicators": {
                    ...
                    "macd": {
                        "current_macd": (
                            round(macd_data["macd"].iloc[-1], 4)
                            if "macd" in macd_data and len(macd_data["macd"]) > 0
                            else None
                        ),
                        "current_signal": (
                            round(macd_data["signal"].iloc[-1], 4)
                            if "signal" in macd_data and len(macd_data["signal"]) > 0
                            else None
                        ),
                ...
                "metadata": {
                    ...
                    "data_quality": (
                        "synthetic" if len(df) == 100 else "real"
    ...
    def _calculate_overall_signal(
        self, rsi_interp: Dict, macd_interp: Dict, ma_analysis: Dict
    ) -> Dict:
        ...
        try:
            ...
            # Взвешенный расчет
            weighted_score = (
                rsi_value * weights["rsi"] + macd_value * weights["macd"] + ma_value * weights["ma"]
            )
    ...
    def format_for_telegram(self, analysis_result: Dict) -> str:
        ...
        if not analysis_result["success"]:
            return (
                f"❌ *Ошибка технического анализа {analysis_result['ticker']}*\n\n"
