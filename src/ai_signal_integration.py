...
class AISignalIntegration:
    ...
    async def analyze_ticker(self, ticker: str) -> AISignal:
        ...
        try:
            ...
            logger.info(
                f"AI анализ {ticker} завершен: {ai_signal.signal_strength.value} "
    ...
    async def _create_combined_signal(
        ...
        combined_score = (
            technical_score * self.technical_weight + news_sentiment_score * self.news_weight
        )
    ...
    def _generate_ai_reasoning(
        ...
        if abs(technical_score) > 0.3:
            ...
            reasoning_parts.append(
                f"Технические индикаторы показывают {tech_direction} сигналы (score: {technical_score:+.2f})"
            )
        else:
        ...
        reasoning_parts.append(
            f"Рекомендация: {signal_strength.value} с уверенностью {confidence:.0%}"
        )
    ...
    def format_signal_for_telegram(self, signal: AISignal) -> str:
        ...
        text += "⚠️ *Дисклеймер:* AI анализ для образовательных целей"
