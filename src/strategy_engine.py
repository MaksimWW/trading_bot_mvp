...
class MACDTrendFollowingStrategy(BaseStrategy):
    ...
    async def generate_signal(self, ticker: str) -> Optional[TradingSignal]:
        ...
        try:
            ...
            logger.info(f"MACD стратегия {ticker}: MACD={macd_value:.3f}, сигнал={action}, уверенность={confidence:.2f}")
...
class StrategyEngine:
    ...
    def start_strategy(self, strategy_id: str, tickers: Optional[List[str]] = None) -> bool:
        ...
        if tickers:
            ...
            if invalid_tickers:
                logger.error(f"Неподдерживаемые тикеры для стратегии {strategy_id}: {invalid_tickers}")
                return False
        
        strategy.start()
        self.active_strategies[strategy_id] = strategy
        
        logger.info(f"Стратегия {strategy_id} запущена для тикеров: {tickers or 'все поддерживаемые'}")
        return True
    ...
    async def execute_strategy_signals(self, ticker: str, auto_execute: bool = False) -> Dict[str, Any]:
        """
    ...
    def get_strategy_performance(self, strategy_id: str) -> Dict[str, Any]:
        ...
        avg_confidence = sum(s.confidence for s in strategy_signals) / total_signals if total_signals > 0 else 0
