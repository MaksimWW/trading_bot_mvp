...
class TradingEngine:
    ...
    def __init__(
        self, mode: TradingMode = TradingMode.PAPER, risk_settings: Optional[RiskSettings] = None
    ):
        """
    ...
    async def generate_trading_signal(self, ticker: str) -> Optional[TradingSignal]:
        ...
        try:
            ...
            news_result = await self.news_analyzer.analyze_ticker_news(
                ticker, include_sentiment=True
            )
            news_score = 0.0
            ...
            if self._is_signal_too_weak(strength) or confidence < self.min_confidence:
                logger.info(
                    f"{ticker}: сигнал слишком слабый ({strength.value}, confidence: {confidence:.2f})"
                )
                return None
    ...
    async def generate_daily_report(self) -> Dict:
        ...
        # Статистика за день
        today_signals = [
            s for s in self.signals_history if s.timestamp.date() == datetime.now().date()
        ]
        ...
        if self.daily_stats["trades_count"] > 0:
            success_rate = (
                self.daily_stats["successful_trades"] / self.daily_stats["trades_count"]
            ) * 100
        ...
        report = {
            ...
            "signals": {
                ...
                "avg_confidence": (
                    sum(s.confidence for s in today_signals) / len(today_signals)
                    if today_signals
                    else 0
                ),
            },
    ...
    def _calculate_levels(
        self, entry_price: float, direction: str, strength: SignalStrength
    ) -> Tuple[float, float]:
    ...
    def _generate_reasoning(
        self, technical_result: Dict, news_result: Dict, combined_score: float
    ) -> str:
    ...
    async def _create_manual_recommendation(
        self, signal: TradingSignal, risk_analysis: Dict
    ) -> Dict:
    ...
    def _update_position(self, position: TradingPosition, current_price: float) -> TradingPosition:
        ...
        if position.entry_price > 0:
            position.unrealized_pnl_percent = (
                position.unrealized_pnl / (position.entry_price * position.shares)
            ) * 100
